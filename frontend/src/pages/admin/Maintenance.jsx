import { useState, useEffect } from "react";
import api from "../../api/axios";
import toast from "react-hot-toast";

const ESTADOS = ["pendiente", "en_proceso", "resuelto"];

export default function Maintenance() {
  const [solicitudes, setSolicitudes] = useState([]);
  const [filtro, setFiltro] = useState("");
  const [notas, setNotas] = useState({});
  const [cargando, setCargando] = useState(true);

  const cargar = async () => {
    setCargando(true);
    try {
      const query = filtro ? "?estado=" + filtro : "";
      const { data } = await api.get("/mantenimiento" + query);
      setSolicitudes(data);
    } catch {
      toast.error("Error al cargar");
    } finally {
      setCargando(false);
    }
  };

  useEffect(() => {
    cargar();
  }, [filtro]);

  const actualizar = async (id, estado) => {
    try {
      await api.put("/mantenimiento/" + id, { estado, nota: notas[id] || "" });
      toast.success("Solicitud actualizada");
      cargar();
    } catch {
      toast.error("Error al actualizar");
    }
  };

  const colorEstado = {
    pendiente: "bg-yellow-100 text-yellow-700",
    en_proceso: "bg-blue-100 text-blue-700",
    resuelto: "bg-green-100 text-green-700",
  };

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-gray-900">Mantenimiento</h1>
        <p className="text-sm text-gray-500">Solicitudes de los residentes</p>
      </div>

      <div className="flex gap-2 flex-wrap">
        {["", ...ESTADOS].map((e) => (
          <button
            key={e}
            onClick={() => setFiltro(e)}
            className={
              "px-3 py-1.5 rounded-full text-sm font-medium transition-colors " +
              (filtro === e
                ? "bg-blue-600 text-white"
                : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50")
            }
          >
            {e || "Todos"}
          </button>
        ))}
      </div>

      {cargando ? (
        <div className="flex justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
        </div>
      ) : (
        <div className="space-y-3">
          {solicitudes.map((s) => (
            <div
              key={s._id}
              className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3"
            >
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-semibold text-gray-900">{s.descripcion}</p>
                  <p className="text-sm text-gray-500">
                    Tipo: {s.tipo} • {s.solicitadoPor?.nombre} • Apto{" "}
                    {s.apartamentoId?.numero}
                  </p>
                  <p className="text-xs text-gray-400">
                    {new Date(s.createdAt).toLocaleDateString("es-DO")}
                  </p>
                </div>
                <span
                  className={
                    "text-xs px-2 py-0.5 rounded-full font-medium " +
                    colorEstado[s.estado]
                  }
                >
                  {s.estado.replace("_", " ")}
                </span>
              </div>

              {s.foto && (
                <a
                  href={s.foto}
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm text-blue-600 hover:underline"
                >
                  Ver foto
                </a>
              )}

              {s.nota && (
                <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-2">
                  📝 {s.nota}
                </p>
              )}

              {s.estado !== "resuelto" && (
                <div className="space-y-2">
                  <input
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Nota interna (opcional)"
                    value={notas[s._id] || ""}
                    onChange={(e) =>
                      setNotas((n) => ({ ...n, [s._id]: e.target.value }))
                    }
                  />
                  <div className="flex gap-2">
                    {s.estado === "pendiente" && (
                      <button
                        onClick={() => actualizar(s._id, "en_proceso")}
                        className="flex-1 bg-blue-600 text-white py-1.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors"
                      >
                        Poner en proceso
                      </button>
                    )}
                    <button
                      onClick={() => actualizar(s._id, "resuelto")}
                      className="flex-1 bg-green-600 text-white py-1.5 rounded-lg text-sm font-medium hover:bg-green-700 transition-colors"
                    >
                      Marcar resuelto
                    </button>
                  </div>
                </div>
              )}
            </div>
          ))}
          {solicitudes.length === 0 && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center text-gray-400">
              <p className="text-3xl mb-2">🔧</p>
              <p className="text-sm">
                No hay solicitudes {filtro && 'con estado "' + filtro + '"'}
              </p>
            </div>
          )}
        </div>
      )}
    </div>
  );
}
